  ...
  return (
    <section id="about" className="py-12 bg-white border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-[auto_1fr] gap-12 lg:gap-24 xl:gap-32 items-start">
        {/* Left Visual Composition - Overlapping Golden-Bordered Photos touching at the corner */}
        <div className="relative w-fit mx-auto lg:mx-0 pt-1 shrink-0">
          {/* Top-Left Photo: Couple looking at blueprint */}
          <div className="w-48 sm:w-56 h-60 sm:h-68 rounded-2xl border-[3.5px] border-brand-navy overflow-hidden shadow-md bg-white relative z-0">
            <img
          ...
          <div className="w-44 sm:w-52 h-56 sm:h-64 rounded-2xl border-[3.5px] border-brand-navy overflow-hidden shadow-xl bg-white -mt-6 sm:-mt-8 ml-28 sm:ml-36 relative z-10">
            <img
              src="/about-keys.jpg"
        ...
        {/* Right Content with maintained generous separation */}
        <div className="flex flex-col items-start lg:pl-4 xl:pl-8">
          <h2
            className="text-[30px] font-normal text-brand-navy uppercase mb-6 tracking-tight pb-[5px]"
          ...
          <p className="text-slate-600 text-sm sm:text-[15px] leading-relaxed mb-4">
            Livnext Home Construction, formerly known as SR Housing and Constructions. With over a decade of expertise in delivering exceptional home construction projects, we are proud to be recognized as a trusted name in the industry.
          </p>

          <p className="text-slate-600 text-sm sm:text-[15px] leading-relaxed mb-4">
            At Livnext, we specialize in turning visions into reality, offering end-to-end solutions for building homes that are not just structures but meaningful spaces that reflect your personality, dreams, and aspirations.
          </p>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full p-6 sm:p-8 relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-5 right-5 text-slate-400 hover:text-slate-700 cursor-pointer" onClick={() => setModalOpen(false)}>
              <X size={24} />
            </button>
            <div className="mb-5">
              <h3 className="text-2xl font-extrabold text-brand-navy mb-1 font-sans">About Livnext Home Construction</h3>
              <p className="text-xs font-semibold text-brand-blue">Excellence in Engineering, Integrity in Every Brick</p>
            </div>
            <div className="text-slate-700 text-sm leading-relaxed">
              <p className="mb-4">
                Formerly known as SR Housing and Constructions, Livnext has pioneered progressive structural designs and premium residential builds across Hyderabad, Telangana, and Andhra Pradesh.
              </p>
              <h4 className="font-bold text-brand-navy text-base mb-3">Our Core Commitments:</h4>
              ...
              <div className="mt-6 pt-5 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 p-4 rounded-xl">
                ...
                <a
                  href="#contact"
                  className="bg-brand-navy hover:bg-brand-blue text-white px-5 py-2.5 rounded-lg text-xs font-bold transition-colors whitespace-nowrap"
